...
Icons are passed to the child components inside the field, not to `Field` itself:

```astro
<Field>
  <Label slot="label" icon="info">Password</Label>
...
- **Label** — label element with optional required indicator and icon.
