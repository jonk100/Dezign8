...
import { HeadingProps } from "~/typography/components/heading/heading.props";
...
Use the `weight` and `size` props to override the default typography values while maintaining semantic levels.

```astro
<Heading level={2} weight="black">Black Weight Heading</Heading>
<Heading level={3} size="xl">Overridden Size Heading</Heading>
```
<div style="border: 1px solid var(--border--default); padding: 1rem; border-radius: var(--radius--md, 8px); margin-bottom: 1rem; display: flex; flex-direction: column; gap: 1rem;">
  <Heading level={2} weight="black">Black Weight Heading</Heading>
  <Heading level={3} size="xl">Overridden Size Heading</Heading>
